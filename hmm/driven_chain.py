...
w_true = np.random.randn(N, u_dim)*2  # weights on the input  ##### explore NxN later???

smooth_lt = 1
stim = np.random.randn(lt)*1
...
def driven_markov_chain(P, weights, ipts, init=None):
    ...
    for t in range(1, n_steps):
        ...
        Pt = np.exp(np.dot(weights[current_state],ipts[t]) + np.log(P[current_state]))
