...
wts0(1,:,1) = [randn(1,nb)*1 0, 1]; %single mGLM
wts0(1,:,2) = [randn(1,nb)*1 0, 1];
...
%  logpy [T K] - loglikelihood for each observation for each state
    ...
    for k = 1:K
        ...
        %%% Gaussian regression
        filt_stim = conv_kernel(stim, K_s) + base;
        logp = -1/(2*1)*(act - filt_stim).^2 - log(1);
        logli(:,k) = [mask.* logp - lambda*sum((K_s - 0).^2)*0]';  % can later add regularization...
    end
end
